...
app.prepare().then(() => {
    ...
    io.on("connection", (socket) => {
        const callbacks = {
            estado: (partida) => {
                io.to(partida.code).emit("partida:estado", (0, gameManager_1.partidaPublica)(partida));
            },
            pregunta: (pregunta, indice, total) => {
                const partida = manager.getPorCode([...socket.rooms].find(r => r !== socket.id) ?? "");
                const code = partida?.code;
                if (code)
                    io.to(code).emit("partida:pregunta", { pregunta, indice, total });
            },
            reveal: (data) => {
        ...
        const conCode = (code) => ({
            estado: (partida) => io.to(partida.code).emit("partida:estado", (0, gameManager_1.partidaPublica)(partida)),
        ...
        socket.on("host:reconectar", ({ code }, cb) => {
            ...
            if (!partida)
                return cb({ ok: false, error: "Partida no encontrada" });
            socket.join(code);
            cb({ ok: true });
            io.to(code).emit("partida:estado", (0, gameManager_1.partidaPublica)(partida));
        });
        socket.on("host:siguiente", ({ code }) => {
            const partida = manager.getPorCode(code);
            if (!partida || partida.hostId !== socket.id)
                return;
        ...
        socket.on("jugador:unirse", ({ code, nombre }, cb) => {
            ...
            io.to(code).emit("partida:estado", (0, gameManager_1.partidaPublica)(res.partida));
        });
